...
mq_recv(void *t)
{
	...
	while(1) {
		...
		if(strcmp(channel, tdata->table.channel_key)) {
			int current_type;
			...
			int new_type;
			if(tdata->table.tuning_space == SPI_UHF || tdata->table.tuning_space == SPI_CATV){
				new_type = CHTYPE_GROUND;
			}else{
				new_type = CHTYPE_SATELLITE;
			}
...
main(int argc, char **argv)
{
	...
	while(1){    // http-server add-
		...
		if(use_http){    // http-server add-
			...
			ssize_t wc = 0;
/* 戻値を受け取るように修正 2025-02-08 AngieKawai-4649 */
			wc = write(connected_socket, header, strlen(header));
			if(wc < 0) {
				fprintf(stderr, "write connect socket error wc:%zd\n", wc);
				return 1;
			}
